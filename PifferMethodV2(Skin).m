...
IQmat={'Luschan skin scale',32,29,27,14,8,14,14,16,29,2,7,23,31,11,27,13,20,30,29,16,18,23,16,28,15,29};
[raw1,raw2,SNP_efdata] = xlsread('EffectDataSkin'); % excel import for effectdata here
SNP_format = dir('SNP_format_skin'); Allele_info = dir('allele_info_skin'); % rename folders here
...
n = 0; m = 1;
if BARon == 'y'
    figure('name','percent fair skin allele in populations by super population')
...
disp '7 - Bargraph of frequency fair skin allele by super population'; 
if BARon == 'y'
...
boxplot(ax,Anovadata,Anovacat); ylim([0 100]); title('Anova boxplot for fair skin allele');
annotation('textbox',dim,'String',cat(2,'P = ',num2str(P_anova)),'FitBoxToText','on');
disp '8 - Anova boxplot of frequency fair skin allele'; disp (cat(2,'P_anova = ',num2str(P_anova))); toc; disp ' ';
...
if BARon == 'n'
    ...
    figure('name','percent fair allele in populations by sub population')
    ...
    for run20 = 1:SNP_number
        if SNP_number > 36
            if n > 35
                figure('name','percent fair skin allele in populations by sub population')
                set(gcf,'Position',[0 0 1950 750])
                n=0; m=m+1;
            end
        end
...
if SNP_number > 28 || BARon == 'n'
    figure('name','Regression analysis of luschan scale vs Polygenic Score')
...
ylim([0 36]); title({'';'Average frequency of fair skin allele in population vs Luschan score'})
annotation('textbox',[0 0.03 0.3 0.3],'String',cat(2,'r_1 (all) = ',num2str(R_polygenicscore)),'FitBoxToText','on');
xlabel('Average frequency positive allele'); ylabel('luschan scale');
...
disp(cat(2,'r_1  for polygenic score vs Luschan score (all) = ',num2str(R_polygenicscore))); toc; disp ' ';
...
rsq = 1 - SSresid/SStotal; R_fa = rsq^0.5; ax = subplot(1,1,1);
hold on
plot(ax,cell2mat(PolygenicScore(:,5)),cell2mat(PolygenicScore(:,4)),'x');
xlabel('FA Score'); ylabel('luschan scale'); ylim([0 36]);
...
annotation('textbox',[0.03 0.55 .3 .3],'String',Fa_loading,'FitBoxToText','on');
annotation('textbox',[0.03 0.58 .3 .3],'String','FA Loading','FitBoxToText','on','EdgeColor','none');
disp '12 - Scatterplot and regression of factor score by sub population';
disp(cat(2,'r_2  for factor score vs luschan score (all) = ',num2str(R_fa))); toc; disp ' ';
